...
3. **Quét mã kiểm thử:** Sử dụng camera của điện thoại quét mã QR hiển thị trên màn hình máy tính để trình duyệt cấp quyền truy cập phần cứng.
4. **Thực thi suy luận thời gian thực:** Trên giao diện trình duyệt điện thoại vừa mở ra, cuộn xuống dưới cùng và bấm chọn nút **"Switch to classification mode"** (Chuyển sang chế độ phân loại). Tiến hành hướng ống kính vào các khối màu lập phương để kiểm chứng độ chính xác (Confidence score đạt từ 0.95 đến 1.00) và tốc độ xử lý biên tối ưu (~1ms).
